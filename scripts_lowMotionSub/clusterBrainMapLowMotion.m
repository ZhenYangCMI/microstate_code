...
for i=1:numSession
    ...
    tmp=load([resultDir,'winFullCorLasso_OptimalLambdaPerSub_645_',session,'_LowMotion.mat']);
    winAllSubAllSeedFullCor=tmp.winFullCorLasso;
    numROI=156;
    
    indx=load([resultDir, session,'_ward_euclidean/lambdaOptimalPerSub/winAllSeeds_FullCorLasso_',session,'_lowMot.txt']);
    ...
    for i=1:numClust
        ...
        save([resultDir,session,'_ward_euclidean/lambdaOptimalPerSub/clusterMean_',num2str(numClust),'clusters_',session,'_lowMotion.mat'],'finalMeanWinOfClust')
        ...
        Header.pinfo = [1;0;0];
        Header.dt    =[16,0];
        rest_WriteNiftiImage(stateMap,Header,[figDir,num2str(numClust),'clusters_state', num2str(i),'_',session,'_lowMotion.nii']);
    end
end
