...
% find the optimal number of clusters for each seed
    ...
    for n=1:N_seed
        ...
        xlim([2 20])
        set(gca,'XTick',2:20,'YTick',0:0.2:1)
...
for n=1:N_seed
        ...
        [indx_stat,ctrs_stat,sumd,D]=kmeans(win_all_sub,N_stat,'Distance','cityblock','emptyaction','singleton','Replicates',5);
        % compute the cluster validity index
    ...
    colorbar('EastOutside')
    caxis([-1,1])
    set(gca,'XTick',1:N_seed*N_stat,'YTick', 1:N_seed*N_stat);
    ylabel('States of all seeds')
    xlabel('States of all seeds')
    title('Correlations of states from all seeds')
    saveas(figure(8),[figdir,'corr_stat_all_seeds.png']);
    ...
    title('State Similarity')
    saveas(figure(9),[figdir,'corr_stat_seed_pair.png']);
    ...
    saveas(figure(10),[figdir,'states.png']);
