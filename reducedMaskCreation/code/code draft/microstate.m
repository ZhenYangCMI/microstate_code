...
for i=1:(asize(1,1)-N+1)
    ...
    s=['cov_' int2str(i) '=' 'TC(' int2str(i) ':' int2str(N) '-1+' int2str(i) ',:)'];
    eval(s)    
    str1='cov_';
    ...
    ic=-L1precisionBCD(oc/mean(diag(atemp)),lambda/1000); % get regularised negative inverse covariance
