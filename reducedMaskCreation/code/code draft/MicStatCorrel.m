...
for i=1:N_seed
    for j=1:50:N_ROI;
        ...
        FC_TC_plot=[zeros(1,ceil(W_width/2)),FC_TC];
        figure(i+4)
...
for i=1:N_seed
    ...
    figure(15+4*(i-1))
    y=vertcat(zeros(ceil(W_width/2),1),indx_final);
    plot(y)
    ylim([0 8])
    xlim([0 N_vol])
...
saveas(figure(i),['/Users/zhenyang/Documents/microstate/figures/', filename,'.png']);
end
