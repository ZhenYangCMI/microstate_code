...
#for (i in sessionNum) {
	#for (j in winTypeNum) {
		baseResults <- sprintf("/home/data/Projects/microstate/DPARSF_preprocessed/results/645/all_10min/%s/surrogate/zNormWindows/", sessionType[i])
		baseFig <- sprintf("/home/data/Projects/microstate/DPARSF_preprocessed/fig/645/all_10min/%s/surrogate/", sessionType[i])

		#fileName <- file.path(baseResults, "test.mat")
		fileName <- file.path(baseResults, sprintf("zWin%sCorLasso_OptimalLambdaPerSub_645_%s_surrogate_%d.mat", winType[j], sessionType[i], surrogateNum))
		...
		# save the index file
	indxFileName <- file.path(baseResults,sprintf("clustIndxWinAllSeeds_%sCorLasso_%s_surrogate_%d.txt", winType[j], sessionType[i], surrogateNum)	)
	write.table(clustIndx,file=indxFileName,row.names=FALSE,col.names=FALSE,qmethod="double")
	
	# plot and save the dendrogram plot
		figName <- file.path(baseFig,sprintf("zWinAllSeeds_%sCorLasso_%s_10min.png",winType[j], sessionType[i]))
