...
winFile <- sprintf("zwinFullCorLasso_OptimalLambdaPerSub_645_%swin%d_%s.mat", sessionType, winSize, covType)
		fileName <- file.path(baseResults, winFile)
		matData <- readMat(fileName)
		# Copy over matrix
		varName <- 'zWinFullCorLasso'
		...
		# save the index file
	indxFileName <- file.path(baseResults,sprintf("clustIndxNormWinAllSeeds_%sCorLasso_%swin%d_%s.txt", winType, sessionType,winSize, covType))
