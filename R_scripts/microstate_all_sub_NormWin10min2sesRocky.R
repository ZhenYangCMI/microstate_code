...
for (k in dataTypeNum) {
		...
		fileName <- file.path(baseResults, sprintf("zWin%sCorLasso_OptimalLambdaPerSub_645_%s_%s.mat", winType[j], sessionType[i],dataType[k]))
		matData <- readMat(fileName)
		# Copy over matrix
		varName <- sprintf("%s",dataType[k])
		...
		# save the index file
	indxFileName <- file.path(baseResults,sprintf("zWinAllSeeds_%sCorLasso_%s_10min_%s.txt", winType[j], sessionType[i], dataType[k])	)
	write.table(clustIndx,file=indxFileName,row.names=FALSE,col.names=FALSE,qmethod="double")
	
	# plot and save the dendrogram plot
		figName <- file.path(baseFig,sprintf("zWinAllSeeds_%sCorLasso_%s_10min_%s.png",winType[j], sessionType[i], dataType[k]))
		png(filename = figName , width = 480, height = 480, units = "px", pointsize = 12,bg = "white")
		figTitle <- sprintf("zWinAllSeeds_%sCorLasso_%s_10min_%s",winType[j], sessionType[i], dataType[k])
