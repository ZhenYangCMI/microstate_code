...
# Read in matlab data
# for winSize34, only 20 subjects were included as more subjects would exceed the R object limit
winSize <-34
...
j <- 1
		...
		fileName <- file.path(baseResults, sprintf("zWinFullCorLasso_OptimalLambdaPerSub_645_2sessions_20sub_win%d_new.mat", winSize))
		matData <- readMat(fileName)
		# Copy over matrix
		varName <- 'zWinFullCorLasso20Sub'
		...
		# save the index file
	indxFileName <- file.path(baseResults,sprintf("clustIndxNormWinAllSeeds_%sCorLasso_%s_10min_20sub_win%d_new.txt", winType[j], sessionType[i], winSize))
	write.table(clustIndx,file=indxFileName,row.names=FALSE,col.names=FALSE,qmethod="double")
	
	# plot and save the dendrogram plot
		figName <- file.path(baseFig,sprintf("zWinAllSeeds_%sCorLasso_%s_10min_20sub_win%d_new.png",winType[j], sessionType[i], winSize))
		png(filename = figName , width = 480, height = 480, units = "px", pointsize = 12,bg = "white")
		figTitle <- sprintf("zWinAllSeeds_%sCorLasso_%s_10min_20sub_win%d",winType[j], sessionType[i], winSize)
