...
for (i in sessionNum) {
	for (j in winTypeNum) {
		fileName <- file.path(base, sprintf("win%sCorLasso_OptimalLambdaPerSub_645_session%i_LowMotion.mat", winType[j], i))
		matData <- readMat(fileName)
		# Copy over matrix
		varName <- sprintf("win%sCorLasso",winType[j])
		...
		# save the index file
	indxFileName <- sprintf("/Users/zhen.yang/Documents/Zhen_CMI/microstate/results/lambdaOptimalPerSub/winAllSeeds_%sCorLasso_%s_lowMot.txt", winType[j], sessionType[i])	
	write.table(clustIndx,file=indxFileName,row.names=FALSE,col.names=FALSE,qmethod="double")
	
	# plot and save the dendrogram plot
		figName <- sprintf("/Users/zhen.yang/Documents/Zhen_CMI/microstate/fig/lambdaOptimalPerSub/winAllSeeds_%sCorLasso_%s_lowMot.png",winType[j], sessionType[i])
		png(filename = figName , width = 480, height = 480, units = "px", pointsize = 12,bg = "white")
		figTitle <- sprintf("winAllSeeds_%sCorLasso_%s_lowMot",winType[j], sessionType[i])
		...
		# plot and save the dendrogram plot
		figName <- sprintf("/Users/zhen.yang/Documents/Zhen_CMI/microstate/fig/lambdaOptimalPerSub/winEachSeed_%sCorLasso_%s_%s_lowMot.png",winType[j], sessionType[i],seed[k])
		png(filename = figName , width = 480, height = 480, units = "px", pointsize = 12,bg = "white")
		figTitle <- sprintf("winEachSeed_%sCorLasso_%s_%s_lowMot",winType[j], sessionType[i],seed[k])
		plot(dendroEachSeed,main=figTitle,ylab="Heights",xlab='FC windows', labels=F)
		dev.off()
			
	}
		# save the index file
		indxFileNameEachSeed <- sprintf("/Users/zhen.yang/Documents/Zhen_CMI/microstate/results/lambdaOptimalPerSub/winEachSeed_%sCorLasso_%s_lowMot.txt", winType[j], sessionType[i])	
