...
if strcmp(mapType, 'corBrainMapAvg')
    ...
    NMax=-1.5; PMax=2;
elseif strcmp(mapType, 'stationaryFCMap')
    NMin=-0.00000000001; PMin = 0.00000000001;
        NMax=-325; PMax=325; 
elseif strcmp(mapType, 'stationaryFCMapR')
    NMin=-0.00001; PMin = 0.00001;
        NMax=-85; PMax=85;
end
...
% cmap1 = colorRamp(ColorMap(1:6,:), 400);
% cmap2= colorRamp(ColorMap(7:end,:), 400);
...
% plot ranked plots
for i=2
    InputName = [imgInputDir,sprintf('thresholdedStationaryFCMapR_seed%d_%s.nii', i, session)];
    
    OutputName = [surfaceMapOutputDir,filesep,sprintf('thresholdedStationaryFCMapR_seed%d_%s.nii', i, session),SurfaceMapSuffix];
...
% 
% 
% OutJPGName=[OutputUpDir,filesep,session, '_', mapType, '_allSeedsCombined.jpg'];
% 
