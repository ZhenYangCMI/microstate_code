...
% Stationalry FC estiamtion: partial correlation
for i=1:numTR
    TR=char(TRList{i});
    for j=1:numSession
        session=char(sessionList{j});
        resultDir=[analyDir,'results/',TR, '/all_10min/',session,'/'];
        partialCorrel=zeros(numSeed,numROI,numSub);
        zStationaryFCPartial=zeros(numSeed, numROI, numSub);
        for k=1:numSub
            sub=subList{k};
            ...
            TCCtrl=zeros(size(TC1,1), numSeed-1);
            for m=1:numSeed
                TCInterest=TC1(:,m);
                p=1;
                for n=1:numSeed
                ...
                %Compute the parital correlation matrix
                partialCorrel(m,:, k) = partialcorr(TCInterest,TC2,TCCtrl);
            end
            for m=1:numSeed
                for p=1:numROI
                    zStationaryFCPartial(m,p,k)=0.5*log(1+partialCorrel(m,p, k))./(1-partialCorrel(m,p,k));
                end
            end
            
            
            % average across all subjects
            tmp1=reshape(zStationaryFCPartial,[],numSub)';
            avg_tmp1=mean(tmp1);
            meanZStationaryFCPartial=reshape(avg_tmp1,numSeed,numROI);
            save([resultDir,'/meanZStationaryFCPartial.mat'],'meanZStationaryFCPartial');
            save([resultDir,'/zStationaryFCPartial.mat'],'zStationaryFCPartial');
            
        end
    end
end

% stationary FC: Full correlation
for i=1:numTR
    TR=char(TRList{i});
    for j=1:numSession
        session=char(sessionList{j});
