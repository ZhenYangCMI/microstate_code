function [seedPrct, subPrct, subSeedPrct] = DoQuickStats(indx,numStates)
...
for i = 1:numStates
%numWinPerState=length(find(indx==i))
    for j = 1:numSeeds
        seedPrct(i,j) = sum(indx(find(seedInd == j)) == i) / (numWin*numSubs); 
    end
    
    for j = 1:numSubs
%subPrct(i,j) = sum(indx(find(subInd == j)) == i) / numWinPerState % norm by the total number of Win per state
        subPrct(i,j) = sum(indx(find(subInd == j)) == i) / (numWin*numSeeds);  % norm by the total number of win per sub
    end
