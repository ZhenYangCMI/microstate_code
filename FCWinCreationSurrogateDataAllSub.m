function [winFullCor, winFullCorLasso, winPartialCorLasso]=FCWinCreationSurrogateDataAllSub(TR,session,subList,lambdaList,surrogateNum)
...
for k=1:numSub
    ...
    numSeed=size(TC1tmp,2);
    
    TC1=zeros(numVol, numRand, numSeed);
    for i=1:numSeed
        seedTS=squeeze(TC1tmp(:,i));
        [ out_ts_arr ] = autocorr_resample_ycg(seedTS, numRand );
        TC1(:,:,i)=out_ts_arr;
    end
    TC1=squeeze(TC1);
    disp('TC was randomized')
...
winFullCor=vertcat(winAllSubSeed1, winAllSubSeed2, winAllSubSeed3, winAllSubSeed4);
...
winPartialCorLasso=vertcat(partWinAllSubSeed1, partWinAllSubSeed2, partWinAllSubSeed3, partWinAllSubSeed4);
...
save([analyDir, 'results/',TR, '/all_10min/',session,'/surrogate/winFullCorLasso_OptimalLambdaPerSub_', TR,'_',session,'_surrogate_', num2str(surrogateNum),'.mat'],'winFullCorLasso')
