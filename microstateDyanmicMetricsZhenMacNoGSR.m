...
numROI=156;
resultDir=['/home/data/Projects/microstate/DPARSF_preprocessed/results/645/', dataLength, filesep, covType, '/', session, '/'];
figDir=['/home/data/Projects/microstate/DPARSF_preprocessed/fig/645/',dataLength, '/', session, '/'];

indx=load([resultDir,'clustIndxNormWinAllSeeds_FullCorLasso_',session,'win69_', covType, '.txt']);
...
if strcmp(session,'session1')
    ...
    indxRecode(find(indx==4))=2;
else
        % these states are recoded as 1-4 and acturally they correspond to
        % state 2 to 5 in session 1 analysis with GSR
    indxRecode(find(indx==1))=4;
    indxRecode(find(indx==2))=2;
    indxRecode(find(indx==3))=1;
    indxRecode(find(indx==4))=3;
    end
...
save([resultDir, 'duration_', session, '_', covType, '.mat'], 'duration')
