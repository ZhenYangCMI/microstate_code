...
tmp=load([resultDir,'/zStationaryFCPartial.mat']);
stationaryFCPartial=tmp.zStationaryFCPartial;
...
for i=1:numSeed
    ...
    for j=1:numSub
        for m=1:numROI
            stationaryFCAllSubOneSeed(j,m)=stationaryFCPartial(i,m,j);
        end
    end
    ...
    Header.pinfo = [1;0;0];
    Header.dt    =[16,0];
    rest_WriteNiftiImage(stationaryFCMap,Header,[figDir,'thresholdedPartialStationaryFCMap_seed',num2str(i),'_', session, '.nii']);
end
