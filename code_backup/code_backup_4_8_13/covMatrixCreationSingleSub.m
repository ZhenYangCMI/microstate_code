function [fullCorrel, fullLasso, partialLasso]=covMatrixCreationSingleSub(TR,session,sub)
...
%3.Sub: in numerical format, e.g. sub=0021006 or in a
%text file.
...
resultDir=[analyDir,'results/',char(TR), '/',session,'/lambdaSensitivity/covMatrix/', sub,'/'];

% create the convolved window
[finalWin]=winCreation(winWidth,0);
finalWin=finalWin';

disp(['Create full and partial correlation matrix for ',char(TR),' ', session,'.'])
...
save([resultDir,'fullCorrel_sub', sub,'.mat'],'fullCorrel')
...
for p=1:numLambda
    ...
    save([resultDir,'fullCovLasso_sub', sub,'_lambda', num2str(lambda),'.mat'],'fullLasso')
    save([resultDir,'inverseCovLasso_sub', sub,'_lambda', num2str(lambda),'.mat'],'partialLasso')
end
end
