...
%                %if(~exist([ses_result_dir,'/avg_z_stat_FC.mat'],'file')
%                 for k=1:N_sub
...
%         end
        ...
        loadfile=load([ses_result_dir,'/z_Dyn_FC.mat'])
...
%                 [indx,ctrs,sumd,D]=kmeans(win_all_sub,N_clust,'Distance','cityblock','emptyaction','singleton','Replicates',5);
%                 
%                 % compute the cluster validity index
%                 CVI(n,N_clust)=sum(sumd)./(sum(sum(D))-sum(sumd));
%             end
%             % opt_N_clust(k,n)=find(CVI(n,2:10)<0.05, 1);
%             
%             figure(8)
%             subplot(2,2,n)
%             plot(CVI(n,:))
...
%             
%         end
%         saveas(figure(8),[figdir, 'CVI_all_win.png']);
%     end
%     save([ses_result_dir,'/opt_N_clust_all_win.txt'],'opt_N_clust','-ascii', '-double', '-tabs');
%     
...
%     appreared solutions across all subjects and all seeds
    ...
    for n=1:N_seed
        ...
        [indx_stat,ctrs_stat,sumd,D]=kmeans(win_all_sub,N_stat,'Distance','correlation','emptyaction','singleton','Replicates',500,'options',opts);
        % compute the cluster validity index
    ...
    save([analysisdir,'results/states.txt'],'ctrs_stat','-ascii', '-double', '-tabs')
    ...
    save([analysisdir,'results/indx_all_sub_all_seeds.txt'],'indx_stat','-ascii', '-double', '-tabs')
    save([analysisdir,'results/sumd_all_sub_all_seeds.txt'],'sumd','-ascii', '-double', '-tabs')
    save([analysisdir,'results/btD_all_sub_all_seeds.txt'],'D','-ascii', '-double', '-tabs')
    ...
    colorbar('EastOutside')
    caxis([-1,1])
    set(gca,'XTick',1:3:N_seed*N_stat,'YTick', 1:3:N_seed*N_stat);
    ...
    for n=1:N_seed
        for t=1:N_stat
            ...
            Header.pinfo = [1;0;0];
            Header.dt    =[16,0];
            rest_WriteNiftiImage(Statemap,Header,[figdir,'seed',num2str(n),'_state', num2str(t),'.nii']);
        end
    end
    end
disp([ses,' done!'])
...
% % find the optimal number of clusters for each seed and each sub
% for k=1:N_sub
%     for n=1:N_seed
...
% end
% save([ses_result_dir,'/opt_N_clust_indiv_sub.txt'],'opt_N_clust','-ascii', '-double', '-tabs');
