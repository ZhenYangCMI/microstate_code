...
for i=1:numTR
    ...
    for j=1:numSes
        ...
        for k=1:numSub
            ...
            asize = size(TC);
            finalWin=WC(1,ceil(winWidth/2):(winWidth+ceil(winWidth/2)-1))';
            % win(W_width,asize(2),N_win_tot);
            for q=1+numWin*(k-1):numWin*k;
                for n=((q-1)-(k-1)*numWin)*step+1:W_width+((q-1)-(k-1)*numWin)*step
                    for m=1:asize(2)
                        win(n-((q-1)-(k-1)*numWin)*step,m,q)=TC(n,m)*finalWin(n-((q-1)-(k-1)*numWin)*step);
                    end
                end
            end
        end
