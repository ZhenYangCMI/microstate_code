...
elseif strcmp(mapType, 'stationaryFCMapR')
    NMin=-0.1; PMin = 0.1;
        NMax=-85; PMax=35;
        % These parameters for set for generate the colorbar
%           NMin=-4; PMin = 4; 
%         NMax=-85; PMax=85;
end
...
% the colorMap used in the paper
...
for i=1:numSeed
    InputName = [imgInputDir,sprintf('thresholdedStationaryFCMap_seed%d_%s.nii', i, session)];
    
    OutputName = [surfaceMapOutputDir,filesep,sprintf('thresholdedStationaryFCMap_seed%d_%s', i, session),SurfaceMapSuffix];
