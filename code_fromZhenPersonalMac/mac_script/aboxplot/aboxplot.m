...
widthl = 0.7;
widths = 1; %box width
widthe = 0.6; % wisker width
outMarker = 'o';
outMarkerSize = 6;
outMarkerEdgeColor = [169/255 169/255 169/255];
outMarkerFaceColor = [1 1 1];
alpha = 0.05;
cmap = [];
colorrev = 0;
colorgrd = 'green_down';
...
for j=1:d
    ...
    for i=1:n
        ...
        line([i-widthl/2+(2*j-1)*widthl/(2*d) i-widthl/2+(2*j-1)*widthl/(2*d)],[fu fl],...
            'Color','k','LineStyle','-','HitTest','off','Parent',hgg(j),'LineWidth',3,'Color',[0,128/255,0]);
        line([i-widthl/2+(2*j-1-widthe)*widthl/(2*d) i-widthl/2+(2*j-1+widthe)*widthl/(2*d)],[fu fu],...
            'Color','k','HitTest','off','Parent',hgg(j),'LineWidth',3,'Color',[0,128/255,0]);
        ...
        if q3 > q1
            rectangle('Position',[i-widthl/2+(2*j-1-widths)*widthl/(2*d) q1 widths*widthl/d q3-q1],...
                'EdgeColor',[0,128/255,0],'FaceColor',colors{j,i},'HitTest','off','Parent',hgg(j),'LineWidth',2);
        end
