...
measure='duration';
subIDList=[1,1,1,1,3,6,10,12];
numSubExclude=length(subIDList);
...
NPdata1=load([resultDir,'DKEFS_WASI_13sub_2sesCmb_session1.txt']);
NPdata2=load([resultDir,'DKEFS_WASI_13sub_2sesCmb_session2.txt']);
...
lineWidth=2.5;
duration1=squeeze(data13sub1(state,seed,:));
duration2=squeeze(data13sub2(state,seed,:));
...
axis square
saveas(figure(2),[figDir, 'scatterPhenotypeDyn_session1_13sub.png'])
...
set(gca,'LineWidth',lineWidth)
axis square
saveas(figure(3),[figDir, 'scatterPhenotypeDyn_session2_13sub.png'])
