...
numROI=156;
resultDir=['/Users/zhenyang/Desktop/Zhen/results/all_10min/',filesep,session,'/'];
...
% compute the totoal number and percentage of windows in a given state
numWinStat=zeros(numState,1);
prctState=zeros(numState,1);
for i=1:numState
numWinStat(i,1)=length(find(indxRecode==i));
prctState(i,1)=numWinStat(i,1)/(numWinPerSeed*numSeed);
end
...
save([resultDir, 'subSeedPrct_', session, '_recode.mat'], 'subSeedPrct')
...
save([resultDir, 'meanProbTM_', session, '_recode.mat'], 'meanProbTM')
save([resultDir, 'probTM_', session, '_recode.mat'], 'probTM')
save([resultDir, 'meanProbTMDifState_', session, '_recode.mat'], 'meanProbTMDifState')
...
numStatesAtATime=4; % 4 possibilities
numUniqStat4Seeds=zeros(numWinPerSub, numSub);
prctDifNumStates=zeros(numSub, numStatesAtATime);
for j=1:numSub
