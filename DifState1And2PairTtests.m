...
meanDif=zeros(1, numROI);
pValue=zeros(1, numROI);
for i=1:numROI
    [h, pTmp]=ttest(meanWin(:, i, 1), meanWin(:, i, 2));
    meanDif(1, i)=mean(meanWin(:, i, 2))-mean(meanWin(:, i, 1)); % meanDif>0 means state 1 greater than state 2: the order was changed to match the concated data.
    pValue(1, i)=pTmp;
end
...
Header.pinfo = [1;0;0];
Header.dt    =[16,0];
rest_WriteNiftiImage(stateMap,Header,[figDir,'thresholdedStatsMap_DifState1AndState2_Pairedtest', session,'_unNormWin.nii']);
